...
class ThermoModel(LCSBModel, Model):
    ...
    def _prepare_metabolite(self, met):
        ...
        if not met.compartment in self.compartments:
            raise Exception("Compartment not found in cobra_model : " + met.compartment)
            ...
            if reaction.thermo["isTrans"]:
                ...
                for met in reaction.metabolites:
                    if met.formula != "H" or (
                        ...
                        DeltaGrxn += reaction.metabolites[met] * met.thermo.deltaGf_tr
                        DeltaGRerr += abs(
    ...
    def _convert_metabolite(self, met, add_potentials, verbose):
        ...
        if metformula == "H2O":
            ...
            LC = self.add_variable(
                LogConcentration, met, lb=log(10**-Comp_pH), ub=log(10**-Comp_pH)
            )
            ...
            if add_potentials:
                ...
                expr = P - self.RT * LC
                self.add_constraint(
                    LogConcentration, "P_" + met.id, expr, metDeltaGF, metDeltaGF
                )
    ...
    def _convert_reaction(self, rxn, add_potentials, add_displacement, verbose):
        ...
        if rxn.thermo["computed"] and not H2OtRxns:
            ...
            if rxn.thermo["isTrans"]:
                ...
                chem_stoich = {met: val for met, val in chem_stoich.items() if val != 0}
                ...
                if add_potentials:
                    ...
                    for met in rxn.metabolites:
                        ...
                        if metformula == "H2O":
                            RHS_DG = RHS_DG + rxn.metabolites[met] * metDeltaGFtr
                        elif metformula != "H":
                    ...
                    for met in rxn.metabolites:
                        ...
                        if metformula not in ["H", "H2O"]:
                            ...
                            LC_ChemMet += self.LC_vars[met] * RT * rxn.metabolites[met]
            ...
            if not NotDrain:
                self.logger.debug(
                    "generating only use constraints for drain reaction" + rxn.id
                )
            else:
                self.logger.debug(
    ...
    def convert(self, add_potentials=False, add_displacement=False, verbose=True):
        ...
        try:
            ...
            raise Exception(
                "Reaction thermo data missing. " + "Please run ThermoModel.prepare()"
            )
    ...
    def copy(self):
        ...
        # To avoid circular import, as ThermoModel is already imported in libs.pytfa.io.dict
        from libs.pytfa.io.dict import model_from_dict, model_to_dict
