...
from libs.pytfa.optim.constraints import NegativeDeltaG
from libs.pytfa.optim.utils import get_solution_value_for_variables, symbol_sum
...
def relax_dgo_gurobi(model, relax_obj_type=0):
    ...
    the_cons = [
        x.constraint._internal_constraint
    ...
    grm = model.solver.problem.feasRelax(
        relaxobjtype=relax_obj_type,
        minrelax=True,
        constrs=the_cons,
        # vars=the_vars,
...
def relax_dgo(tmodel, reactions_to_ignore=(), solver=None, in_place=False):
    ...
    for this_neg_dg in tqdm(my_neg_dg, desc="adding slacks"):
        ...
        # the constraint
        neg_slack = slack_model.add_variable(
            NegSlackVariable, this_neg_dg.reaction, lb=0, ub=BIGM_DG, queue=False
        )
        pos_slack = slack_model.add_variable(
    ...
    neg_slack_values = get_solution_value_for_variables(relaxation, my_neg_slacks)
    pos_slack_values = get_solution_value_for_variables(relaxation, my_pos_slacks)
    ...
    for this_reaction in tqdm(relaxed_model.reactions, desc="applying slack"):
        ...
        # Get the relaxation
        dgo_delta_lb = neg_slack_values[my_neg_slacks.get_by_id(this_reaction.id).name]
        dgo_delta_ub = pos_slack_values[my_pos_slacks.get_by_id(this_reaction.id).name]
    ...
    if len(changes) == 0:
        ...
        tmodel.logger.error(
            "Relaxation could not complete " "(no DeltaG relaxation found)"
        )
...
def relax_lc(tmodel, metabolites_to_ignore=(), solver=None):
    ...
    for this_lc in my_lc:
        ...
        neg_slack[this_lc.name] = slack_model.add_variable(
            NegSlackLC, this_lc, lb=0, ub=BIGM_DG
        )

        pos_slack[this_lc.name] = slack_model.add_variable(
            PosSlackLC, this_lc, lb=0, ub=BIGM_DG
        )
    ...
    for this_met in relaxed_model.metabolites:
        ...
        # Get the relaxation
        lc_delta_lb = neg_slack_values[my_neg_slacks.get_by_id(this_met.id).name]
        lc_delta_ub = pos_slack_values[my_pos_slacks.get_by_id(this_met.id).name]
