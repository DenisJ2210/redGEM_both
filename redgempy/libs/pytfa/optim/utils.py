...
from libs.pytfa.optim.constraints import GenericConstraint
...
THERMO_VARIABLE_PREFIXES = ("DG_", "DGo_", "LC_")
...
def symbol_sum(variables):
    ...
    if isinstance(variables[k], GenericVariable):
        ...
        isinstance(variables[k], optlang.interface.Variable)
        or isinstance(variables[k], sympy.Mul)
        or isinstance(variables[k], sympy.Add)
        or isinstance(variables[k], Number)
    ):
        return Add(*variables)
    else:
        raise ValueError(
            "Arguments should be of type Number, sympy.Add, or sympy.Mul, "
...
def get_solution_value_for_variables(solution, these_vars, index_by_reaction=False):
    if isinstance(these_vars[0], GenericVariable):
        ...
        raise TypeError(
            """<these_vars> should be of type pytfa.solving.Variable,
...
def get_active_use_variables(tmodel, solution):
    ...
    :return:
    """
    use_variables = tuple(tmodel.get_variables_of_type(BackwardUseVariable)) + tuple(
        tmodel.get_variables_of_type(ForwardUseVariable)
    )
...
def get_direction_use_variables(tmodel, solution):
    ...
    return [
        (
            fwd_use_variables.get_by_id(x.id)
            if solution.raw[x.id] > epsilon
...
def get_primal(tmodel, vartype, index_by_reactions=False):
    ...
    if index_by_reactions:
        return pd.Series({x.id: x.variable.primal for x in the_vars})
    else:
        return pd.Series({x.name: x.variable.primal for x in the_vars})
...
def strip_from_integer_variables(tmodel):
    ...
    print(
        "Is the cobra_model still integer ? {}".format(
            continuous_model.solver.is_integer
        )
    )
...
def strip_from_thermo_variables(tmodel):
    """
    Removes all integer and binary variables from a cobra_model to make it sample-able.

    Parameters:
    tmodel : cobra.Model
        The thermodynamic model from which to remove integer and binary variables.

    Returns:
    cobra.Model
        A continuous version of the input model with integer, binary, and thermo variables removed.
    """
    continuous_model = tmodel
    continuous_model.name = tmodel.name + " - continuous"
    ...
    # Iterate over all constraints and check for integer variables
    for this_cons in continuous_model.constraints:
        has_integer_variable = False
        for this_var in this_cons.variables:
            if this_var.type in INTEGER_VARIABLE_TYPES:
                has_integer_variable = True
                this_var_descriptor = continuous_model.variables.get(this_var.name)
                integer_variables.add(this_var_descriptor)
        if has_integer_variable:
            constraints_with_integer_variables.append(this_cons)

    # Remove constraints that include integer variables
    for this_cons in constraints_with_integer_variables:
        continuous_model.remove_constraint(this_cons)
    ...
    # Remove thermo variables
    thermo_variables = set()
    constraints_with_thermo_variables = []

    # Iterate over all constraints and check for thermo variables
    for this_cons in continuous_model.constraints:
        has_thermo_variable = False
        for this_var in this_cons.variables:
            if this_var.name.startswith(THERMO_VARIABLE_PREFIXES):
                has_thermo_variable = True
                this_var_descriptor = continuous_model.variables.get(this_var.name)
                thermo_variables.add(this_var_descriptor)
        if has_thermo_variable:
            constraints_with_thermo_variables.append(this_cons)

    # Remove constraints and variables that include thermo variables
    for this_cons in constraints_with_thermo_variables:
        continuous_model.remove_constraint(this_cons)

    for this_var in thermo_variables:
        continuous_model.remove_variable(this_var)
    ...
    continuous_model.remove_reactions(rxns_to_remove)
