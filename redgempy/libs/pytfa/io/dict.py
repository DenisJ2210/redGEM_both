...
def model_from_dict(obj, solver=None, custom_hooks=None):
    ...
    for the_var_dict in obj["variables"]:
        ...
        if classname in get_model_variable_subclasses():
            ...
            raise TypeError("Class {} serialization not handled yet".format(classname))
    ...
    for the_cons_dict in obj["constraints"]:
        ...
        new_expr = parse_expr(
            the_cons_dict["expression"], local_dict=variable_parse_dict
        )
        ...
        if classname in get_model_constraint_subclasses():
            ...
            this_class = name2class[classname]
            nc = new.add_constraint(
                kind=this_class, hook=hook, expr=new_expr, ub=ub, lb=lb, queue=True
            )
        else:
            raise TypeError("Class {} serialization not handled yet".format(classname))
...
def get_hook_dict(model, custom_hooks):
    ...
    return {
        classname: getattr(model, hookname)
...
def rebuild_compositions(new, compositions_dict):
    ...
    return {
        k: _rebuild_stoichiometry(new, stoich)
...
def _rebuild_stoichiometry(new, stoich):
    ...
    return defaultdict(
        int, {new.metabolites.get_by_id(k): v for k, v in stoich.items()}
    )
